...
struct Router {
    ...
    static func viewController(_ viewModel: ViewModelProtocol) -> ViewProtocol?  {
        ...
        let vcName = viewModel.vcName
        let viewController = router.viewControllerFromClassName(clsName: vcName)
...
extension Router {
    ...
    private func viewControllerFromClassName(clsName: String) -> ViewProtocol? {
        let cls = NSClassFromString(clsName) as? UIViewController.Type
        if let cls = cls {
            let vc = cls.init()
            return vc as? ViewProtocol
        }else {
            return nil
        }
